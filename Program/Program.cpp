...
#pragma region 복사 생성자
	...
	//Object object1(10, 20); // Object(int width, int height) 함수로 진입

	//object1.Address();

	//Object object2 = object1; // object2(object1);과 동일 => Object(Object & object) 함수로 진입
...
#pragma region 얕은 복사
	// 객체를 복사할 때 주소 값을 복사하여 같은 메모리를 가리키는 복사입니다.
	...
	//// 하나의 객체로 값을 변경하게 되면 서로 참조된 객체도 함께 영향을 받게 됩니다.
	...
	//delete ptr1;
	//delete ptr2; // (X) 이미 해제된 메모리를 또 해제하므로

	// 얕은 복사는 객체로 메모리에서 해제될 때
	// 생성자의 실행 순서와 반대로 소멸자가 호출되기 때문에 복사한 객체가 먼저 해제되므로
