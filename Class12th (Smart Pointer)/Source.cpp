...
#pragma region RAII 패턴 (Resoruce Acquisition is Initialization)
// 자원의 안전한 사용을 위해
// 객체가 쓰이는 범위를 벗어나게 되면
// 자원을 해제해주는 기법입니다.
...
#pragma region weak 포인터
	...
	// expired라는 상태가 되는데 이는 원시 포인터의 참조 카운트가 0이 되어 메모리가 해제되었으므로
	// 이 원시 포인터를 소유한 weak 포인터를 유효하지 않다고 판단하여 해제하는 포인터입니다.
...
#pragma region unique 포인터
	/* // 하나의 객체만 가리킬 수 있는 스마트 포인터입니다.

	std::unique_ptr<File> uniquePtr1(new File());
	// (using namespace std;가 있으면 'std::' 적을 필요 없음)
	...
	// unique_ptr<File> uniquePtr3 = uniquePtr1; (X)

	// 하지만 객체에 대한 소유권을 이전하는 것은 가능합니다.
	unique_ptr<File> uniquePtr3 = std::move(uniquePtr2);
	// (using namespace std;가 있으면 'std::' 적을 필요 없음)
